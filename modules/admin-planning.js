...
function updateViewButtons() {
    document.querySelectorAll('.view-btn').forEach(btn => {
        ...
        btn.style.backgroundColor = isSelected ? 'var(--color-surface)' : 'transparent';
...
function displayDayView() {
    ...
    planningGrid.innerHTML = `<div class="p-2 rounded-lg flex flex-col w-full" style="background-color: var(--color-background);"><div class="flex justify-between items-center mb-2"><div class="flex items-center gap-2 flex-wrap">${days.map((day, index) => `<button data-day-index="${index}" class="day-selector-btn px-3 py-1 text-sm rounded-md ${index === state.selectedDayIndex ? 'text-white shadow' : ''}" style="background-color: ${index === state.selectedDayIndex ? 'var(--color-primary)' : 'var(--color-surface)'};">${day}</button>`).join('')}</div><button data-date="${dateString}" class="add-chantier-btn text-2xl font-bold hover:opacity-70" style="color: var(--color-primary);">+</button></div><div class="day-tasks-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-2 flex-grow overflow-y-auto" style="grid-auto-flow: column; grid-template-rows: repeat(9, auto);" id="day-col-${dateString}"></div></div>`;
...
function populateTeamPool() {
    ...
    state.teamMembers.forEach(member => {
        ...
        item.innerHTML = `<input type="checkbox" class="team-checkbox h-4 w-4 rounded border-gray-300 focus:ring-offset-0" style="color: var(--color-primary);" data-member-id="${member.id}"><span class="text-sm">${member.name}</span>`;
...
function renderTeamInBlock(planningBlock, teamNames) {
    ...
    teamZone.innerHTML = (teamNames && teamNames.length > 0) ? teamNames.map(name => `<div class="team-member-tag p-0.5 rounded text-[10px] leading-tight flex items-center gap-1" data-name="${name}" title="Cliquer pour retirer" style="background-color: var(--color-background);"><span>${name}</span><span class="font-bold text-red-500 hover:text-red-700 pointer-events-none">&times;</span></div>`).join('') : '';
}
...
async function savePlanningItem(closeAfterSave) {
    ...
    try {
        if (state.editing.id) {
            const planningRef = doc(db, "planning", state.editing.id);
            await updateDoc(planningRef, dataToSave);
            const originalTaskData = await getPlanningTask(state.editing.id);
            const updatedTaskData = { ...originalTaskData, ...dataToSave };
            const existingBlock = document.querySelector(`.planning-block[data-planning-id="${state.editing.id}"]`);
            if (existingBlock) {
                existingBlock.replaceWith(createChantierBlock(updatedTaskData));
            }
        } else {
            ...
            const dataWithMeta = { ...dataToSave, date: state.editing.date, teamNames: [], order: snapshot.size, createdAt: serverTimestamp() };
            const newDocRef = await addDoc(collection(db, "planning"), dataWithMeta);
            const newPlanningDoc = { id: newDocRef.id, ...dataWithMeta };
            const container = document.getElementById(`day-col-${state.editing.date}`);
            if (container) {
                container.appendChild(createChantierBlock(newPlanningDoc));
            }
        }
        if (closeAfterSave) {
...
async function logChangeForUser(userId, changeObject, batch = null) {
    if (!userId) return;
    const userRef = doc(db, "users", userId);
    const updateData = { pendingChanges: arrayUnion(changeObject) };
    if (batch) {
...
async function handleMemberRemoval(planningBlock, planningDoc, memberNameToRemove) {
    ...
    renderTeamInBlock(planningBlock, planningDoc.teamNames);
    await updateDoc(doc(db, "planning", planningDoc.id), { teamNames: planningDoc.teamNames });
    const member = state.teamMembers.find(m => m.name === memberNameToRemove);
    if (member) {
        const change = {
            date: planningDoc.date,
            type: 'retrait',
        ...
        await logChangeForUser(member.id, change);
    }
}
...
function logDeletionForTeam(batch, taskData) {
    if (!taskData.teamNames || taskData.teamNames.length === 0) return;
    const change = {
        date: taskData.date,
        type: 'suppression',
        chantier: taskData.chantierName,
        timestamp: new Date().toISOString()
    };
    taskData.teamNames.forEach(name => {
        const member = state.teamMembers.find(m => m.name === name);
        if (member && member.id) {
            logChangeForUser(member.id, change, batch);
        }
    });
}
...
async function deleteSelectedItems() {
    if (state.selectedItems.size === 0) {
        showInfoModal("Information", "Aucun élément sélectionné.");
        return;
    }
    if (await showConfirmationModal("Confirmation", `Supprimer les ${state.selectedItems.size} éléments sélectionnés ?`)) {
        const batch = writeBatch(db);
        const tasksToNotify = await Promise.all(Array.from(state.selectedItems).map(id => getPlanningTask(id)));
        
        tasksToNotify.forEach(task => {
            if (task) {
                logDeletionForTeam(batch, task);
                batch.delete(doc(db, "planning", task.id));
            }
        });
        
        await batch.commit();
        showInfoModal("Succès", `${state.selectedItems.size} éléments supprimés.`);
        toggleSelectionMode(); // Also resets items
        await display();
    }
}
...
async function checkAndRenderPublishButton(startOfWeek) {
    ...
    if (publishDoc.exists()) {
        ...
        btn.className = 'bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-lg text-sm';
        btn.onclick = () => showInfoModal("Information", "Les notifications sont maintenant envoyées automatiquement à chaque modification (ajout/retrait/suppression).");
    } else {
        btn.textContent = 'Publier la semaine';
        btn.className = 'bg-green-600 hover:bg-green-700 text-white font-bold px-4 py-2 rounded-lg text-sm';
...
async function publishWeek() {
    ...
    if (await showConfirmationModal("Publication", `Voulez-vous PUBLIER le planning pour la semaine du ${weekString} ? Les modifications ne généreront plus de notifications.`)) {
        try {
            ...
            showInfoModal("Succès", "Planning publié. Les notifications automatiques pour cette semaine sont désactivées.");
            await checkAndRenderPublishButton(startOfWeek);
        } catch (error) {
            console.error(error);
            showInfoModal("Erreur", "La publication a échoué.");
        }
    }
}

async function sendUpdateNotification() {
    showInfoModal("Information", "Les notifications sont maintenant envoyées automatiquement à chaque modification (ajout/retrait/suppression). Ce bouton n'est plus nécessaire.");
}